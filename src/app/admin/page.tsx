    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold font-mono text-zinc-900 dark:text-zinc-100">Telemetry & Source Operations</h1>
          </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            disabled={isSyncing}
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-white hover:bg-zinc-100 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-zinc-800 dark:text-zinc-200 border border-zinc-200 dark:border-zinc-700 text-xs font-mono font-medium transition-colors disabled:opacity-50 shadow-sm"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        <div className="p-4 rounded-xl bg-white dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 shadow-sm">
          ...
          <div className="text-2xl font-bold font-mono text-zinc-900 dark:text-zinc-100 mt-1">{metrics.totalJobs}</div>
        </div>

        <div className="p-4 rounded-xl bg-white dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 shadow-sm">
          <span className="text-[11px] font-mono text-zinc-500">ACTIVE IN STREAM</span>
          <div className="text-2xl font-bold font-mono text-emerald-600 dark:text-emerald-400 mt-1">{metrics.activeJobs}</div>
        </div>

        <div className="p-4 rounded-xl bg-white dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 shadow-sm">
          <span className="text-[11px] font-mono text-zinc-500">CLOSED / EXPIRED</span>
          <div className="text-2xl font-bold font-mono text-rose-600 dark:text-rose-400 mt-1">{metrics.closedJobs}</div>
        </div>

        <div className="p-4 rounded-xl bg-white dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 shadow-sm">
          <span className="text-[11px] font-mono text-zinc-500">DUPLICATES CLUSTERED</span>
          <div className="text-2xl font-bold font-mono text-cyan-600 dark:text-cyan-400 mt-1">{metrics.duplicateJobsDetected}</div>
        </div>
      </div>
      ...
      <div className="mb-10 rounded-xl bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm">
        <div className="p-5 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-mono font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-wider">Source Health Monitor</h2>
          </div>
          <span className="text-xs font-mono text-zinc-500 dark:text-zinc-400">{sources.filter((s) => s.isEnabled).length} of {sources.length} Active</span>
        </div>

        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs font-mono">
            <thead className="bg-zinc-50 dark:bg-zinc-900/60 text-zinc-600 dark:text-zinc-400 border-b border-zinc-200 dark:border-zinc-800">
              <tr>
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/60 text-zinc-700 dark:text-zinc-300">
              {sources.map((s) => (
                <tr key={s.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/40 transition-colors">
                  <td className="p-3.5 font-semibold text-zinc-900 dark:text-zinc-200">
                    {s.name}
                  </td>
                  <td className="p-3.5 text-zinc-500 dark:text-zinc-400">{s.type}</td>
                  <td className="p-3.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      {s.status === "DELAYED" && (
                        <>
                          ...
                          <span className="text-amber-600 dark:text-amber-400 font-bold">Delayed</span>
                        </>
                      )}
                      {s.status === "ERROR" && (
                        <>
                  ...
                  <td className="p-3.5 text-zinc-500 dark:text-zinc-400">{s.errorCount}</td>
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800/60 text-zinc-700 dark:text-zinc-300">
              {recentCrawlRuns.slice(0, 8).map((r) => (
                <tr key={r.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/40">
                  <td className="p-3.5 font-semibold text-zinc-900 dark:text-zinc-200">{r.sourceName}</td>
                  <td className="p-3.5 text-zinc-500 dark:text-zinc-400">{new Date(r.startedAt).toLocaleTimeString()}</td>
